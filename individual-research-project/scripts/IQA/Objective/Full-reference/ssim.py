...
pixel_area_relation_ssim_array = []
...
def plot_bar_chart(mean_nearest_neighbor, mean_bilinear, mean_bicubic, mean_lanczos, mean_pixel_area_relation):
    ...
    # Save the figure
    plt.savefig('SSIM_bar_chart.png')
    # Show the plot
    # plt.show()
    # Indication of plotting bar chart completed
    print("SSIM: Bar chart has been drawn")
...
def main():
    ...
    for i in range(25):
        try:
            ...
            ssim_value_nearest, _ = ssim(reference_grayscale, test_grayscale, full=True)
            # Append name and data into arrays
            nearest_neighbor_name_array.append(input_image_files[i])
            nearest_neighbor_ssim_array.append(ssim_value_nearest)
    ...
    for i in range(25):
        try:
            ...
            if reference_grayscale.shape != test_grayscale.shape:
                ...
                test_grayscale = cv2.resize(test_grayscale, (reference_grayscale.shape[1], reference_grayscale.shape[0]))
            # Calculate Structural similarity index
            ssim_value_bilinear, _ = ssim(reference_grayscale, test_grayscale, full=True)
            # Append name and data into arrays
            bilinear_name_array.append(input_image_files[i])
            bilinear_ssim_array.append(ssim_value_bilinear)
        except FileNotFoundError as e:
            # Handle file not found exception
            print(f"File not found: {e}")
        except Exception as e:
    ...
    for i in range(25):
        try:
            ...
            if reference_grayscale.shape != test_grayscale.shape:
                ...
                test_grayscale = cv2.resize(test_grayscale, (reference_grayscale.shape[1], reference_grayscale.shape[0]))
            # Calculate Structural similarity index
            ssim_value_bicubic, _ = ssim(reference_grayscale, test_grayscale, full=True)
            # Append name and data into arrays
            bicubic_name_array.append(input_image_files[i])
            bicubic_ssim_array.append(ssim_value_bicubic)
        except FileNotFoundError as e:
            # Handle file not found exception
            print(f"File not found: {e}")
        except Exception as e:
    ...
    for i in range(25):
        try:
            ...
            if reference_grayscale.shape != test_grayscale.shape:
                ...
                test_grayscale = cv2.resize(test_grayscale, (reference_grayscale.shape[1], reference_grayscale.shape[0]))
            # Calculate Structural similarity index
            ssim_value_lanczos, _ = ssim(reference_grayscale, test_grayscale, full=True)
            # Append name and data into arrays
            lanczos_name_array.append(input_image_files[i])
            lanczos_ssim_array.append(ssim_value_lanczos)
        except FileNotFoundError as e:
            # Handle file not found exception
            print(f"File not found: {e}")
        except Exception as e:
    ...
    for i in range(25):
        try:
            ...
            if reference_grayscale.shape != test_grayscale.shape:
                ...
                test_grayscale = cv2.resize(test_grayscale, (reference_grayscale.shape[1], reference_grayscale.shape[0]))
            # Calculate Structural similarity index
            ssim_value_pixel, _ = ssim(reference_grayscale, test_grayscale, full=True)
            # Append name and data into arrays
            pixel_area_relation_name_array.append(input_image_files[i])
            pixel_area_relation_ssim_array.append(ssim_value_pixel)
        except FileNotFoundError as e:
            # Handle file not found exception
            print(f"File not found: {e}")
        except Exception as e:
    ...
    for i in range(25):
        tmp_nearest_neighbor = tmp_nearest_neighbor + nearest_neighbor_ssim_array[i]
    mean_nearest_neighbor = tmp_nearest_neighbor/25
    # Bilinear
    tmp_bilinear = 0.0
    for i in range(25):
        tmp_bilinear = tmp_bilinear + bilinear_ssim_array[i]
    mean_bilinear = tmp_bilinear/25
    # Bicubic
    tmp_bicubic = 0.0
    for i in range(25):
        tmp_bicubic = tmp_bicubic + bicubic_ssim_array[i]
    mean_bicubic = tmp_bicubic/25
    # Lanczos
    tmp_lanczos = 0.0
    for i in range(25):
        tmp_lanczos = tmp_lanczos + lanczos_ssim_array[i]
    mean_lanczos = tmp_lanczos/25
    # Pixel area relation
    tmp_pixel_area_relation = 0.0
    for i in range(25):
        tmp_pixel_area_relation = tmp_pixel_area_relation + pixel_area_relation_ssim_array[i]
    ...
    ssim_data = {
        'Processing metrics' : [
            ...
            'Pixel area relation + GANs (+ Bilinear)',
            'Pixel area relation + GANs (+ Bilinear)',
            'Average SSIM: Pixel area relation'
        ],
        'Index' : [
            # Nearest-neighbor
            '001',
        ...
        'Structural Similarity Index' : [
            # Nearest-neighbor
            nearest_neighbor_ssim_array[0],
            nearest_neighbor_ssim_array[1],
            nearest_neighbor_ssim_array[2],
            nearest_neighbor_ssim_array[3],
            nearest_neighbor_ssim_array[4],
            nearest_neighbor_ssim_array[5],
            nearest_neighbor_ssim_array[6],
            nearest_neighbor_ssim_array[7],
            nearest_neighbor_ssim_array[8],
            nearest_neighbor_ssim_array[9],
            nearest_neighbor_ssim_array[10],
            nearest_neighbor_ssim_array[11],
            nearest_neighbor_ssim_array[12],
            nearest_neighbor_ssim_array[13],
            nearest_neighbor_ssim_array[14],
            nearest_neighbor_ssim_array[15],
            nearest_neighbor_ssim_array[16],
            nearest_neighbor_ssim_array[17],
            nearest_neighbor_ssim_array[18],
            nearest_neighbor_ssim_array[19],
            nearest_neighbor_ssim_array[20],
            nearest_neighbor_ssim_array[21],
            nearest_neighbor_ssim_array[22],
            nearest_neighbor_ssim_array[23],
            nearest_neighbor_ssim_array[24],
            mean_nearest_neighbor,
            # Bilinear
            bilinear_ssim_array[0],
            bilinear_ssim_array[1],
            bilinear_ssim_array[2],
            bilinear_ssim_array[3],
            bilinear_ssim_array[4],
            bilinear_ssim_array[5],
            bilinear_ssim_array[6],
            bilinear_ssim_array[7],
            bilinear_ssim_array[8],
            bilinear_ssim_array[9],
            bilinear_ssim_array[10],
            bilinear_ssim_array[11],
            bilinear_ssim_array[12],
            bilinear_ssim_array[13],
            bilinear_ssim_array[14],
            bilinear_ssim_array[15],
            bilinear_ssim_array[16],
            bilinear_ssim_array[17],
            bilinear_ssim_array[18],
            bilinear_ssim_array[19],
            bilinear_ssim_array[20],
            bilinear_ssim_array[21],
            bilinear_ssim_array[22],
            bilinear_ssim_array[23],
            bilinear_ssim_array[24],
            mean_bilinear,
            # Bicubic
            bicubic_ssim_array[0],
            bicubic_ssim_array[1],
            bicubic_ssim_array[2],
            bicubic_ssim_array[3],
            bicubic_ssim_array[4],
            bicubic_ssim_array[5],
            bicubic_ssim_array[6],
            bicubic_ssim_array[7],
            bicubic_ssim_array[8],
            bicubic_ssim_array[9],
            bicubic_ssim_array[10],
            bicubic_ssim_array[11],
            bicubic_ssim_array[12],
            bicubic_ssim_array[13],
            bicubic_ssim_array[14],
            bicubic_ssim_array[15],
            bicubic_ssim_array[16],
            bicubic_ssim_array[17],
            bicubic_ssim_array[18],
            bicubic_ssim_array[19],
            bicubic_ssim_array[20],
            bicubic_ssim_array[21],
            bicubic_ssim_array[22],
            bicubic_ssim_array[23],
            bicubic_ssim_array[24],
            mean_bicubic,
            # Lanczos
            lanczos_ssim_array[0],
            lanczos_ssim_array[1],
            lanczos_ssim_array[2],
            lanczos_ssim_array[3],
            lanczos_ssim_array[4],
            lanczos_ssim_array[5],
            lanczos_ssim_array[6],
            lanczos_ssim_array[7],
            lanczos_ssim_array[8],
            lanczos_ssim_array[9],
            lanczos_ssim_array[10],
            lanczos_ssim_array[11],
            lanczos_ssim_array[12],
            lanczos_ssim_array[13],
            lanczos_ssim_array[14],
            lanczos_ssim_array[15],
            lanczos_ssim_array[16],
            lanczos_ssim_array[17],
            lanczos_ssim_array[18],
            lanczos_ssim_array[19],
            lanczos_ssim_array[20],
            lanczos_ssim_array[21],
            lanczos_ssim_array[22],
            lanczos_ssim_array[23],
            lanczos_ssim_array[24],
            mean_lanczos,
            # Pixel area relation
            pixel_area_relation_ssim_array[0],
            pixel_area_relation_ssim_array[1],
            pixel_area_relation_ssim_array[2],
            pixel_area_relation_ssim_array[3],
            pixel_area_relation_ssim_array[4],
            pixel_area_relation_ssim_array[5],
            pixel_area_relation_ssim_array[6],
            pixel_area_relation_ssim_array[7],
            pixel_area_relation_ssim_array[8],
            pixel_area_relation_ssim_array[9],
            pixel_area_relation_ssim_array[10],
            pixel_area_relation_ssim_array[11],
            pixel_area_relation_ssim_array[12],
            pixel_area_relation_ssim_array[13],
            pixel_area_relation_ssim_array[14],
            pixel_area_relation_ssim_array[15],
            pixel_area_relation_ssim_array[16],
            pixel_area_relation_ssim_array[17],
            pixel_area_relation_ssim_array[18],
            pixel_area_relation_ssim_array[19],
            pixel_area_relation_ssim_array[20],
            pixel_area_relation_ssim_array[21],
            pixel_area_relation_ssim_array[22],
            pixel_area_relation_ssim_array[23],
            pixel_area_relation_ssim_array[24],
            mean_pixel_area_relation
        ]
    }

    plot_table(data=ssim_data)
