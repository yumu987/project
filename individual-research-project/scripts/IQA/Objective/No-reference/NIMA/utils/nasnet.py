...
####################
# keras.engine.topology does not support Python 3.7.9
# get_source_inputs() function has been defined in this script
####################
...
####################
# from keras.engine.topology import get_source_inputs
# original function from keras.engine.topology
####################
def get_source_inputs(target, for_summary=False, rank_nested=False):
    """
    Retrieves the input tensors of a Keras layer or model.

    Parameters:
    - target: A Keras layer or model.
    - for_summary: If True, include layers in Model's summary.
    - rank_nested: If True, return nested layer inputs as a flat list.

    Returns:
    - List of input tensors.
    """
    if hasattr(target, 'nested_layers'):
        if rank_nested:
            return [x for x in target._nested_inputs if x in target.inputs]
        else:
            return [x for x in target._nested_inputs if x in target.inputs]

    if isinstance(target, list):
        return [x for x in target if x in target.inputs]

    if isinstance(target, dict):
        return [x for x in target.values() if x in target.inputs]

    return [target] if hasattr(target, '_keras_history') else []
####################
...
def _adjust_block(p, ip, filters, weight_decay=5e-5, id=None):
    ...
    with K.name_scope('adjust_block'):
        ...
        ####################
        # The _keras_shape does not support the type
        ####################
